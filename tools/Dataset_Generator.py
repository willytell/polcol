...
class Dataset_Generator(keras.utils.Sequence):
    def __init__(self, cf, dataset_images_path, n_classes=2, batch_size=5, resize_image=(224, 224), shuffle_dataset=False, seed=1925, 
                 shuffle_batch=False, seed_batch=55, apply_augmentation=False, sampling_score=None, data_path='data', data_path2=None, mode='train'):
    ...
    def __len__(self):
        ...
        return int(self.total_images // self.batch_size)
    ...
    def on_epoch_end(self):
        ...
        if self.mode == 'test' or self.mode == 'validation':
            ...
            if self.shuffle_dataset:
                shuffle(self.X_global, self.y_global, self.seed)

        if self.mode == 'train':           
            if self.shuffle_dataset:
                shuffle(self.X_noneo, self.y_noneo_class, self.seed)
                shuffle(self.X_neop, self.y_neop_class, self.seed)
                self.make_train_set()
    ...
    def __data_generation(self):
        ...
        if self.shuffle_batch:
            shuffle(self.batch_fnames, self.batch_labels, self.seed_batch)
        ...
        return (np.array(img_batch), np.array(lab_batch))


    # strategy 1: keep unbalanced each batch as the dataset.
    def __getitem__(self, index):
        self.batch_fnames = self.X_global[index * self.batch_size:(index + 1) * self.batch_size]
        self.batch_labels = self.y_global[index * self.batch_size:(index + 1) * self.batch_size]
