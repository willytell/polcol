...
then
    srun --pty --gres=gpu:1,gmem:12G --mem 8G python3 main.py -c $CONFIG_PATH/conf_exp1-sm.py -a train #&>logs/vgg-from-scratch-train 

elif [ "$1" = "test" ]
then
    srun --pty --gres=gpu:1,gmem:12G --mem 8G python3 main.py -c config/conf_exp1-sm.py -a test #&>logs/vgg-from-scratch-test
else
